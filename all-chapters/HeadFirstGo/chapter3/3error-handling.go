...
func perimeter(wid float64, hei float64) (float64, error) {
	if wid < 0 {
		return 0, fmt.Errorf("Negative length is invalid!:%v", wid)
	}
	if hei < 0 {
		return 0, fmt.Errorf("Negative height is invalid!:%v", hei)
	}
...
func main() {
	...
	fmt.Println("You will need", total, "meters of fencing.")
}
